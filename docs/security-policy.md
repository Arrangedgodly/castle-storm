...
| 6 | **Platform host** (future T-PERF-01; today `ui/main.gd` is a stub) | The only layer permitted to read a clock: it reads the OS clock, converts to UTC epoch, and injects it into surface #2/#3. | `ui/` contains zero clock reads on GAME paths (code-scan asserted); the one exception of record is spread_screen's env-gated capture hook, whose wall-ms measurement (`Time.get_ticks_msec` — T-UI-09's foreground→actionable evidence) sits behind `CS_SPREAD_*` dev chrome, never on a play path; the boundary is the gdscript-conventions determinism rule ("time arrives as tick counts or injected timestamps"). |

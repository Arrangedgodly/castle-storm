...
| **Card-title clip below the plate-fit floor** | The phone-scale title/role clip (this sweep's fail-safe) is now the LAST resort: the backlog sweep added the card grammar's plate fit (`CardFace.fit_label_to_width` — a label steps its font down to its plate before clipping, floored), so titles/roles fit at 2–3-column rosters AND at the 5-column 108px plate for the longest pool name (font-metric pinned in `test_theme_grammar`). The RESIDUE is structural: at plate widths where even the floor size cannot fit the longest names, the label still clips at the plate edge (fail-SAFE, never past the card) — pinned as the documented state, `test_face_plates_step_down_to_fit_before_clipping`. |

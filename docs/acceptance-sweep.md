...
| **Card-title clip below the plate-fit floor** | RESOLVED (the readability pass, 2026-09-17): the 55%/70% fit floors are GONE — `CardFace.fit_label_to_width` now steps a plate's font down until the WHOLE text fits (shrink-to-full-fit, absolute 8px floor; the whole ladder also raised one coherent step, spread_theme.tres 34/22/19/15 → 36/24/22/18). The clip fail-safe is never engaged by real copy at any roster density the layout grants; at the densest rosters the prints are whole-but-small (density, not clip). Pinned by `test_face_plates_step_down_to_fit_before_clipping` (108px plate AND the hostile 60px stub both fit whole). |

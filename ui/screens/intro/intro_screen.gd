...
##              the same regime + "the regime remembers"). RESTART
##              VARIANTS DEAL THE NEW LEADER THEMSELVES: the run has
##              ended by the time this opens, so the screen submits the
##              real run_restart + grant verbs through the host's one
##              write path and drains the tick — the identity the reveal
##              prints IS the identity the sim drew (a fresh leader
##              always; a redrawn regime after victory, the same regime
##              after defeat — the sim's own rules, never re-implemented
##              here). The run_restarted events flow through the host's
##              unified feed, so the spread beneath rebuilds itself;
...
##              whole time, so the unfold IS the load;
...
## presenter re-derives the truth from the actual chronicle.
##
...
## path: the host already started the first run).
func open(p_host: GameHost, p_router: LayoutRouter, p_variant: StringName = &"") -> void:
	host = p_host
	_router = p_router
	variant = p_variant
	if variant != IntroPresenter.VARIANT_FIRST_RUN and not host.is_run_running():
		host.restart_run()
		host.advance_ticks(1)
	_view = IntroPresenter.reveal_view(host)
	variant = _view["variant"]
	state = State.REVEAL
	visible = true
	interactions = 0
	_packet.bind(_view)
	_packet._chip.grab_focus.call_deferred()
...
func unfold() -> void:
	...
	last_unfold_seconds = IntroPresenter.unfold_seconds()
